...
// func checkForUpdates(c *gin.Context, h *Handlers) {
// 	deviceID := c.Query("deviceId")
// 	clientLastUpdate := c.Query("lastUpdate")

// 	if deviceID == "" || clientLastUpdate == "" {
// 		c.JSON(http.StatusBadRequest, gin.H{"error": "Missing parameters"})
// 		return
// 	}

// 	clientTime, err := time.Parse(time.RFC3339, clientLastUpdate)
// 	if err != nil {
...
// 	h.UpdateMutex.RLock()
// 	serverLastUpdate, exists := h.LastUpdateTimes[deviceID]
...
// 	if clientTime.Before(serverLastUpdate) {
...
// 	router.GET("/check-updates", func(c *gin.Context) { checkForUpdates(c, handlers) })
